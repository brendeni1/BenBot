...
CONVERSATION_INACTIVITY_TIMEOUT = (
    30  # Minutes for a conversation to be considered inactive.
)
INACTIVITY_WARNING = (
    (CONVERSATION_INACTIVITY_TIMEOUT / 4) * 60
    if CONVERSATION_INACTIVITY_TIMEOUT >= 1
    else 0
)  # Seconds for a conversation warning to be sent in an inactive thread.
...
ACCEPTED_FILE_TYPES = {
    "Gemini": {
        "Documents": [
            ...
            "text/xml",
            "text/rtf; charset=utf-8" "text/rtf",
        ],
        "Scripts": [
...
def typesByModel(model: str) -> list | None:
    try:
        acceptableTypes = [
            fileType
            for category in ACCEPTED_FILE_TYPES[model].values()
            for fileType in category
        ]
...
async def parseAttachments(
    ctx: discord.ApplicationContext, model: str, attachments: list[discord.Attachment]
    ...
    for attachment in attachments:
        if attachment.content_type in acceptableTypes:
            ...
            rejectedAttachments.append(
                (
                    attachment,
                    f"File {attachment.filename} ({attachment.content_type}) not in supported types. Use /ai filetypes for a list of supported file types.",
                )
            )

    if rejectedAttachments:
        reply = EmbedReply(
            f"{model} - Attachments - Error",
            "ai",
            True,
            description=f"Some attachments were not parsed and therefore ignored:",
        )
...
async def sendEndingWarning(thread: discord.Thread, warnAfter: int, bot: discord.Bot):
    try:
        ...
        if peopleWhoSpoke:
            peopleWhoSpoke = filter(
                lambda member: member.id != bot.user.id, peopleWhoSpoke
            )

        warning = EmbedReply(
            "AI Conversation - Timeout Warning",
            "ai",
            description=f"⚠️ You have {formattedLeft} left before the conversation times out!\n\nUse /ai timeoutreset or interact with the AI to keep the conversation for another {dates.formatSeconds(int(CONVERSATION_INACTIVITY_TIMEOUT * 60))}.",
        )
...
def deleteGoogleMedia(threadID: int):
    try:
        ...
        for safeName in namesToDelete:
            ...
            print(
                f"GOOGLE AI LOG: Deleted data {safeName} for {threadID} when closing AI conversation."
            )

        del activeGoogleSafeNames[threadID]

        print(
            f"GOOGLE AI LOG: All data for {threadID} was deleted when closing AI conversation."
        )
    except KeyError as e:
        print(
            f"GOOGLE AI LOG: No data for {e} was deleted when closing AI conversation."
        )
    except Exception as e:
        raise e
...
class Ai(commands.Cog):
    ...
    ai = discord.SlashCommandGroup(
        "ai",
        "A collection of commands for prompting AI models.",
        guild_ids=[799341195109203998],
    )

    @ai.command(
        description="Converse with an LLM. Use /ai prompt for single requests. Attachments supported.",
        guild_ids=[799341195109203998],
    )
    async def conversation(
        ...
        if len(prompt) < MINIMUM_PROMPT_LENGTH:
            reply = EmbedReply(
                ...
                "ai",
                True,
                description=f"Please provide a prompt at least {MINIMUM_PROMPT_LENGTH} characters long.",
            )
        ...
        if model == "Gemini":
            ...
            chat = googleClient.chats.create(
                model="gemini-2.0-flash", config=chatConfig
            )
        else:
            reply = EmbedReply(
                "AI - Error",
                "ai",
                True,
                description="You picked an invalid model somehow.",
            )
        ...
        if not isinstance(ctx.channel, discord.Thread):
            ...
            reply = EmbedReply(
                "AI - Conversation",
                "ai",
                description=f"Created new thread for this conversation.",
            )
            ...
            thread = await origMessage.create_thread(
                name=threadTitle, auto_archive_duration=60
            )
        ...
        if model in ACCEPTED_FILE_TYPES and ACCEPTED_FILE_TYPES[model]:
            threadInstructions = EmbedReply(
                f"{model} - Conversation",
                "ai",
                description=f"You have started a new conversation with {model}. Attachments supported with this model, simply send whatever in this thread.\n\nReply within this thread to continue the conversation.\n\nTo end this conversation, delete this thread or use /ai end. This thread will also close after {dates.formatSeconds(int(CONVERSATION_INACTIVITY_TIMEOUT * 60))} of inactivity.",
            )
        else:
            threadInstructions = EmbedReply(
                f"{model} - Conversation",
                "ai",
                description=f"You have started a new conversation with {model}. Attachments not supported with this model.\n\nReply within this thread to continue the conversation.\n\nTo end this conversation, delete this thread or use /ai end. This thread will also close after {dates.formatSeconds(int(CONVERSATION_INACTIVITY_TIMEOUT * 60))} of inactivity.",
            )
                ...
                return all(
                    [
                        (
                            ...
                            and TIMEOUT_RESET_SUCCESS == message.embeds[0].description
                        )
        ...
        closeConversationReply = EmbedReply(
            "AI - Conversation Closed",
            "ai",
            description=f"Conversation with {model} started by {ctx.author.mention} has timed out and the thread was deleted.",
        )
        ...
        try:
            async with thread.typing():
                if model == "Gemini":
                    await asyncio.to_thread(
                        chat.send_message,
                        f"{SYSTEM_MESSAGE} You can also accept attachments. If the user doesn't know what attachments they can use, tell them to use the '/ai filetypes' command.",
                    )
                    initialPrompt = await asyncio.to_thread(
                        chat.send_message, f"{prompt}"
                    )

                    splitForDiscord = text.truncateString(
                        initialPrompt.text, 4096, splitOnMax=True
                    )

                    for num, chunk in enumerate(splitForDiscord, 1):
                        initialPromptReply = EmbedReply(
                            f"{model} - Reply - ({num}/{len(splitForDiscord)})",
                            "ai",
                            description=chunk,
                        )
            ...
            reply = EmbedReply(
                ...
                "ai",
                True,
                description=f"Error in conversation when replying to prompt: {e}",
            )

            await reply.send(ctx)
        except Exception as e:
            reply = EmbedReply(
                f"{model} - Error", "ai", True, description=f"Error: {e}"
            )
        ...
        while thread.id in activeThreads:
            ...
            if INACTIVITY_WARNING:
                warningTimer = asyncio.create_task(
                    sendEndingWarning(thread, INACTIVITY_WARNING, self.bot)
                )

            try:
                nextMessage: discord.Message = await self.bot.wait_for(
                    "message",
                    check=conversationCheck,
                    timeout=CONVERSATION_INACTIVITY_TIMEOUT * (60),
                )
                ...
                async with thread.typing():
                    if model == "Gemini":
                        parsedAttachments = await parseAttachments(
                            thread, model, nextMessage.attachments
                        )
                        ...
                        if not nextMessage.content and parsedAttachments:
                            ...
                            content = [
                                "Prompt the user that you could not use the file and tell them to only submit valid filetypes using the /ai filetypes command."
                            ]
                        ...
                        if parsedAttachments:
                            for originalAttachment in parsedAttachments:
                                    ...
                                    urllib.parse.quote(
                                        (
                                            re.sub(
                                                ...
                                                "-",
                                                (
                                                    f"{random.randint(0, 1000)}-{(originalAttachment.filename).lower()}"
                                                ),
                                            )
                                        ).strip("-")
                                    )
                                ...
                                uploadedFile = googleClient.files.upload(
                                    ...
                                    config={
                                        "name": googleSafeName,
                                        "mime_type": originalAttachment.content_type,
                                    },
                                )
                        ...
                        splitForDiscord = text.truncateString(
                            response.text, 4096, splitOnMax=True
                        )
                        ...
                        raise ValueError(
                            "Nonexistent model name in conversation prompt."
                        )
                ...
                reply = EmbedReply(
                    "AI - Error", "ai", True, description=f"Error in thread: {e}"
                )

                await thread.send(embed=reply)

        print(
            f"CONVERSATION LOG: Terminated/Exited conversation while loop with thread ID: {thread.id}"
        )

    @ai.command(
        description="End a conversation with an LLM. Only usable inside of threads invoked by /ai conversation.",
        guild_ids=[799341195109203998],
    )
        ...
        try:
            if isinstance(ctx.channel, discord.Thread):
                if ctx.channel.id in activeThreads:
                    ...
                    reply = EmbedReply(
                        "AI - End Conversation - Error",
                        "ai",
                        True,
                        description="This thread cannot be closed because is not registered as an active AI conversation by the bot. You will need to delete this yourself.",
                    )
                ...
                reply = EmbedReply(
                    ...
                    "ai",
                    True,
                    description="This command can only be used in a thread which was created by the /ai conversation command and is registered as an active AI conversation by the bot.",
                )
            ...
            reply = EmbedReply(
                ...
                "ai",
                True,
                description=f"Error occured when ending thread: {e}",
            )

            await reply.send(ctx)

    @ai.command(
        description=f"Reset the {CONVERSATION_INACTIVITY_TIMEOUT} min AI conversation inactivity timer.",
        guild_ids=[799341195109203998],
    )
    async def timeoutreset(self, ctx: discord.ApplicationContext):
        try:
            if isinstance(ctx.channel, discord.Thread):
                if ctx.channel.id in activeThreads:
                    thread = self.bot.get_channel(ctx.channel.id)
                    ...
                    reply = EmbedReply(
                        "AI - Timeout Reset", "ai", description=TIMEOUT_RESET_SUCCESS
                    )

                    await reply.send(ctx)
                else:
                    reply = EmbedReply(
                        "AI - Timeout Reset - Error",
                        "ai",
                        True,
                        description="This thread's timeout cannot be reset because is not registered as an active AI conversation by the bot. You will need to make a new conversation.",
                    )
            ...
            reply = EmbedReply(
                ...
                "ai",
                True,
                description=f"Error occured when resetting thread timeout: {e}",
            )

            await reply.send(ctx)

    @ai.command(
        description=f"Archive an AI conversation and spare deletion. You won't be able to interact with the model.",
        guild_ids=[799341195109203998],
    )
        ...
        try:
            if isinstance(ctx.channel, discord.Thread):
                if ctx.channel.id in activeThreads:
                    ...
                    reply = EmbedReply(
                        "AI - Archive", "ai", description=ARCHIVE_SUCCESS
                    )
                    ...
                    reply = EmbedReply(
                        ...
                        "ai",
                        True,
                        description="This thread cannot be archived because is not registered as an active AI conversation by the bot. You will need to make a new conversation.",
                    )
    ...
    @ai.command(
        description="Prompt an LLM. Use /ai conversation for multiple prompts or image/file support.",
        guild_ids=[799341195109203998],
    )
    async def prompt(
        self,
        ctx: discord.ApplicationContext,
        prompt: discord.Option(
            str, description="Prompt to send to an LLM.", required=True
        ),  # type: ignore
        model: discord.Option(
            str,
            description="AI model to use. Default: Google Gemini.",
        ...
        try:
            ...
            reply = EmbedReply(
                ...
                "ai",
                True,
                description=f"Error when replying to prompt: {e}",
            )
    ...
    @ai.command(
        description="Show a list of accepted file types for AI conversations.",
        guild_ids=[799341195109203998],
    )
    async def filetypes(
        self,
        ...
        try:
            ...
            if not acceptedTypes:
                reply = EmbedReply(
                    f"{model} - File Types - Error",
                    "ai",
                    True,
                    description=f"{model} does not support attachments.",
                )
            ...
            reply = EmbedReply(
                f"{model} - File Types",
                "ai",
                description=f"The following file types are acceptable for use with {model}:",
            )
            ...
            reply = EmbedReply(
                "AI - File Types - Error", "ai", True, description=f"Error: {e}"
            )
